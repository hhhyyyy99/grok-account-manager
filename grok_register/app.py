...
from grok_register.paths import (
    PROJECT_ROOT,
    CONFIG_FILE as _CONFIG_PATH,
    CRASH_LOG_FILE as _CRASH_PATH,
    TURNSTILE_DIR,
    TOKEN_JSON,
    create_batch_output_dir,
    current_output_path,
    get_active_batch_dir,
)
...
def cloudflare_create_temp_address(api_base):
    ...
    if is_admin_create:
        payload = {"name": generate_email_username(10), "enablePrefix": True}
        if domain:
...
def _normalize_grok2api_remote_pool(pool_name):
    value = str(pool_name or "").strip() or "ssoBasic"
    mapping = {
        "ssoBasic": "basic",
        "ssoSuper": "super",
        "basic": "basic",
...
def _local_grok2api_pool_name(remote_pool):
    value = str(remote_pool or "").strip().lower() or "basic"
    mapping = {
        "basic": "ssoBasic",
        "super": "ssoSuper",
        "ssobasic": "ssoBasic",
...
def _coerce_grok2api_token_pools(payload, preferred_pool="ssoBasic"):
    """Normalize GET /tokens payload into {local_pool_name: [items]}."""
    ...
    if isinstance(tokens, dict):
        ...
        for key, items in tokens.items():
            pools[_local_grok2api_pool_name(key)] = list(items or [])
        return pools
    if isinstance(tokens, list):
        pools = {}
        for item in tokens:
            if isinstance(item, dict):
                local_name = _local_grok2api_pool_name(item.get("pool") or preferred_pool)
                pools.setdefault(local_name, []).append(item)
            elif isinstance(item, str):
                token = _normalize_sso_token(item)
                if token:
                    pools.setdefault(preferred_pool, []).append({"token": token})
        return pools
    return None


def _remote_pool_save_payload(pools):
    """Convert local pool map into remote save payload keyed by basic/super."""
    payload = {}
    for local_name, items in (pools or {}).items():
        remote_name = _normalize_grok2api_remote_pool(local_name)
        cleaned = []
        for item in items or []:
            if isinstance(item, dict):
                token = _normalize_sso_token(item.get("token", ""))
                if not token:
                    continue
                entry = {"token": token}
                tags = item.get("tags")
...
def append_mail_credential(email, credential):
    path = current_output_path("mail_credentials.txt")
...
def append_account_record(path, email, password, sso):
...
def create_browser_options(*, unique_profile: bool = False, profile_tag: str = "reg"):
    ...
    DrissionPage auto_port gives every options object an isolated profile. The
    compatibility arguments remain because callers use them to document intent.
    """
    ...
    for flag in CHROMIUM_SLIM_FLAGS:
        try:
            ...
            pass
    # auto_port allocates both a unique debugging port and a temporary profile.
    try:
        options.auto_port()
    except Exception:
        pass
...
def http_put(url, **kwargs):
    try:
        ...
        if "127.0.0.1 port 7890" in err or "Could not connect to server" in err:
            ...
            retry_kwargs["proxies"] = {}
            try:
                return curl_requests.put(url, **_build_request_kwargs(**retry_kwargs))
            except Exception as retry_exc:
                if _is_tls_backend_error(retry_exc):
                    return std_requests.put(url, **_to_std_request_kwargs(retry_kwargs))
                raise
        raise
...
def yyds_get_email_and_token(api_key=None, jwt=None):
    ...
    username = generate_email_username(10)
...
def get_email_prefix():
    raw = str(config.get("email_prefix", "") or "").strip().lower()
    return "".join(ch for ch in raw if ch in string.ascii_lowercase + string.digits + "-_")


def generate_email_username(random_length=10):
    return f"{get_email_prefix()}{generate_username(random_length)}"
...
def tk_button(parent, text="", command=None, state=None, **kwargs):
    if tk is None:
        raise RuntimeError("Tk is unavailable in this Python environment")
    if state is None:
        state = tk.NORMAL
    return tk.Button(
        parent,
    ...
    for attempt in range(1, 5):
        ...
        try:
            ...
            try:
                ...
                pass
            if browser is None and opts is not None:
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        add_label(10, 0, "邮箱前缀（可选）:")
        self.email_prefix_var = tk.StringVar(value=str(config.get("email_prefix", "")))
        self.email_prefix_entry = tk_entry(config_frame, textvariable=self.email_prefix_var, width=34)
        add_field(self.email_prefix_entry, 10, 1)
    ...
    def start_registration(self):
        ...
        config["email_prefix"] = self.email_prefix_var.get().strip()
        ...
        self.accounts_output_file = str(batch_dir / "accounts.txt")
        self.update_stats()
        self._set_running_ui(True)
        self.log(f"[*] 配置已保存，开始执行。目标数量: {count}")
        self.log(f"[*] 本次批次目录: {batch_dir}")
...
def run_registration_cli(count):
    ...
    max_slot_retry = 3
    batch_dir = create_batch_output_dir()
    accounts_output_file = str(batch_dir / "accounts.txt")
    cli_log(f"[*] 终端模式启动，目标数量: {count}")
    cli_log(f"[*] 本次批次目录: {batch_dir}")
